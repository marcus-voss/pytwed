...
class PyTwedTest(TestCase):
    ...
    def test_pytwed_constrained_fast(self):
        s1 = np.array([[0.], [0.], [1.], [1.], [2.], [3.], [5.], [2.], [0.], [1.], [-0.1]]).astype('float64')
        s2 = np.array([[0.], [1.], [2.], [2.5], [3.], [3.5], [4.], [4.5], [5.5], [2.], [0.], [0.], [.25], [.05], [0.]]).astype('float64')
        s3 = np.array([[4.], [4.], [3.], [3.], [3.], [3.], [2.], [5.], [2.], [.5], [.5], [.5]]).astype('float64')
        ...
        assert(round(twed(s1, s2, ts1, ts2, nu=nu, lmbda=lmbda, p=p, fast=True, radius=2), 3) <= round(twed(s1, s2, ts1, ts2, nu=nu, lmbda=lmbda, p=p, fast=True), 3))  
        assert(round(twed(s1, s3, ts1, ts3, nu=nu, lmbda=lmbda, p=p, fast=True, radius=2), 3) <= round(twed(s1, s3, ts1, ts3, nu=nu, lmbda=lmbda, p=p, fast=True), 3))  
        assert(round(twed(s2, s3, ts2, ts3, nu=nu, lmbda=lmbda, p=p, fast=True, radius=2), 3) <= round(twed(s2, s3, ts2, ts3, nu=nu, lmbda=lmbda, p=p, fast=True), 3))  
        ...
        ts5 = np.array([1., 2., 3., 4., 5.]).astype('float64')
          ...
          # test that constraint is always less

        assert(round(twed(s4, s5, ts4, ts5, nu=nu, lmbda=lmbda, p=p, fast=True, radius=2), 3) <= round(twed(s4, s5, ts4, ts5, nu=nu, lmbda=lmbda, p=p, fast=True), 3))  
    ...
    def test_pytwed_flat_fast(self):
        ...
        assert_almost_equal(twed(s4, s5, nu=nu, lmbda=lmbda, p=p, fast=True), 24.904, decimal=3)
    ...
    def test_pytwed_flat_slow(self):
        ...
        assert_almost_equal(twed(s4, s5, nu=nu, lmbda=lmbda, p=p, fast=False), 24.904, decimal=3)
